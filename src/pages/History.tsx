...
import { type AttendanceRow, ensureAdminContext, getUserProfile, supabase } from "@/lib/supabase";
...
const HistoryPage = () => {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data, error }) => {
      ...
      setUserEmail(data.user.email ?? "Operatore");

      const { data: historyData, error: historyError } = await supabase
        .from("attendance_events")
        .select("id, association_id, child_id, action, location_name, created_at, children(full_name)")
        .eq("association_id", adminContext.association.id)
      ...
      setLogs((historyData ?? []) as AttendanceRow[]);
      setLoading(false);
    });
      ...
      <header className="sticky top-0 z-20 border-b border-white/60 bg-slate-50/95 px-4 py-4 backdrop-blur-sm sm:px-6">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="hidden rounded-full bg-white px-4 py-2 text-sm font-bold text-slate-500 shadow-sm sm:block">{userEmail || "Storico"}</div>
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-sky-50 px-4 py-2 text-[11px] font-black uppercase tracking-[0.28em] text-sky-700">
                ...
                Storico amministratore
              </div>
              <h1 className="mt-5 text-3xl font-black text-slate-900 sm:text-4xl">Check-in e check-out dell’associazione</h1>
              <p className="mt-3 max-w-2xl text-sm font-medium text-slate-500 sm:text-base">
                Qui trovi gli ultimi movimenti registrati per tutti i bambini dell’associazione, inclusi centro attivo e orario preciso.
              </p>
            </div>
        ...
        {loading ? (
          ...
          <section className="grid gap-4">
            {logs.map((log) => {
              ...
              const actionClasses = log.action === "check_in" ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700";

              return (
                <article key={log.id} className="flex flex-col gap-4 rounded-[2rem] border border-slate-200/70 bg-white p-5 shadow-lg shadow-slate-200/50 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-col gap-2 text-sm font-medium text-slate-500 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
                        <span className="flex items-center gap-2">
                          ...
                          {new Intl.DateTimeFormat("it-IT", { dateStyle: "medium", timeStyle: "short" }).format(createdAt)}
                        </span>
                        <span className="flex items-center gap-2">
                  ...
                  <div className={`inline-flex w-fit rounded-full px-4 py-2 text-[11px] font-black uppercase tracking-[0.22em] ${actionClasses}`}>{actionLabel}</div>
                </article>
              );
            })}
          </section>
          ...
          <Card className="rounded-[2rem] border-none bg-white shadow-lg shadow-slate-200/60">
            <CardContent className="flex flex-col items-center justify-center px-6 py-12 text-center">
              ...
              <p className="mt-2 max-w-md text-sm font-medium text-slate-500">Quando gli operatori inizieranno a registrare le presenze, lo storico apparirà qui in tempo reale.</p>
            </CardContent>
          </Card>
        )}
      </main>
